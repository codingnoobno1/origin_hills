...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (isSandboxMode()) return NextResponse.json({ success: true, message: "Product updated (sandbox)", isSandbox: true });
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    if (isSandboxMode()) return NextResponse.json({ success: true, message: "Product deleted (sandbox)", isSandbox: true });
